...
/**
 *  在多玩Model层中专门设置的头文件，目的只要一个，方便其他类引入多玩的全部头文件
 */
...
/**
 *  获取免费英雄或收费英雄列表，因为免费和收费英雄请求串十分相似，所以和写
 *
 *  @param type 请求英雄类型
 *
 *  @return 当前请求所在任务
 ...
 *  @return 网络请求任务
 */
+ (id)getHeroSoundWithHeroName:(NSString *)enName kCompletionHandle;
...
//** 获取游戏百科列表 */
+ (id)getToolMenukCompletionHandle:(void(^)(id model, NSError *error))completionHandle;

//** 获取装备分类 */
+ (id)getZBCategorykCompletionHandle:(void(^)(id model, NSError *error))completionHandle;
